...
Read the [kernel contract](kernel-contract.md), [note operations](note-operations.md), and [research guidance](research-to-pkm.md). Inspect enough local conventions to place and format the output; avoid a whole-vault redesign discussion.
...
For files, images, audio, video, creator channels, playlists, or relevant mixed media, read [explore-media.md](explore-media.md). Inspect the actual modalities and preserve their locators and access limitations. Channel exploration uses a bounded, explained sample of content. Transcripts, OCR, and page metadata are not interchangeable with visual or audio understanding. For code or documentation, identify the relevant version or repository revision where available.
...
- For media, which modalities and time/page ranges were actually inspected; for channels/playlists, the sample and selection rationale.
...
Read [explore-capture.md](explore-capture.md) and use `scripts/explore_capture.py` for new captures. Supply the actually researched body and source ledger; the helper validates citation IDs, coverage declarations, filenames, duplicates, properties, and local links. It is a local writer, not a crawler or fact checker.
